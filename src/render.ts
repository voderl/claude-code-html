...
.hint b { color: #fff; }
/* "generated by …" credit line at the bottom of the page. Sits below the
   transcript content (block-level, padded), not as a fixed overlay, so it
   doesn't compete with the floating hint / menu chrome. Muted colour so
   it doesn't pull attention away from the captured terminal output. */
.ccs-credit {
  /* Outer wrapper: kept at body's font-size so var(--cols, …ch) computes
     against the same 1ch as the transcript pane (otherwise an 11px font on
     this element would shrink ch and the credit wouldn't span the column).
     --cols is set on body by the responsive picker in JS each time a
     template is cloned; falls back to 100% before the first paint. */
  width: var(--cols, 100%);
  height: 0px;
  margin-top: 12px;
  text-align: center;
...
.ccs-credit-inner {
  /* Inner span carries the actual visual text style — small grey monospace,
     applied here rather than on .ccs-credit so changing font-size doesn't
     affect the parent's ch-based width. */
  font: 11px/1.4 ui-monospace, monospace;
  color: #6a6a6a;
}
.ccs-credit-inner a { color: #9a9a9a; text-decoration: none; }
.ccs-credit-inner a:hover { color: #d0d0d0; text-decoration: underline; }
...
<div class="ccs-credit"><span class="ccs-credit-inner">generated by <a href="https://github.com/voderl/claude-code-html" target="_blank" rel="noopener noreferrer">voderl/claude-code-html</a></span></div>
...
(function () {
  ...
  function render() {
    ...
    // Expose the picked template's cols on body so the credit (a sibling of
    // #snap) can size itself to match the transcript pane width.
    document.body.style.setProperty('--cols', tpl.dataset.cols + 'ch');
    applyMode();
  }
  function applyMode() {
